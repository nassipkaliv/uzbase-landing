...
function PricingCard({ tier, isPopular, popularLabel }: {
  tier: { name: string; price: string; description: string; features: string[]; cta: string };
  isPopular: boolean;
  popularLabel: string;
}) {
  ...
  return (
    ...
    >
      ...
      <p className="text-3xl lg:text-4xl font-bold text-foreground mt-3">{tier.price}</p>
      <p className="text-muted text-sm mt-2 mb-6">{tier.description}</p>
      ...
      <Button as="a" href="#contact" variant={isPopular ? "primary" : "outline"} className="w-full">
        {tier.cta}
      </Button>
    </div>
  );
          ...
          <div className="grid md:grid-cols-3 gap-5 items-start">
            {t.pricing.tiers.map((tier, index) => (
              <PricingCard
                key={index}
                tier={tier}
                isPopular={TIER_META[index].highlighted}
